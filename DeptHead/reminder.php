...
// Assuming user ID and department ID are stored in the session
$user_id = $_SESSION['user_id'] ?? null; // Ensure this is set
$user_dept_id = $_SESSION['user_dept_id'] ?? null;

// Check if user is logged in and department ID is available
if (!$user_id || !$user_dept_id) {
    echo json_encode(['error' => 'User not logged in or department not assigned.']);
    exit;
}

// Check if department_ids are being sent for fetching content
if (isset($_POST['department_ids'])) {
    ...
    if (!empty($department_ids)) {
        ...
        $types = str_repeat('i', count($department_ids)); // Prepare types for binding
...
// Query to get the total number of reminders for pagination calculation
