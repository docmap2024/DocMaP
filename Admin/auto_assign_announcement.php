...
function write_log($message) {
    $logfile = '/tmp/logfile.log'; // Log file path
...
function autoAssignTasks() {
    ...
    $sql = "SELECT TaskID, ContentID, UserID, Type, Title, taskContent, Schedule_Date, Schedule_Time, DueDate, DueTime FROM tasks WHERE Type = 'Announcement' AND Status = 'Schedule'";
...
if ($mobileResult->num_rows > 0) {
    ...
    while ($row = $mobileResult->fetch_assoc()) {
        ...
        $messages[] = "NEW ANNOUNCEMENT ALERT!\n\nHi " . $row['FullName'] . "! " . $userName . " Posted a new Announcement! ($contentTitle) \"" . $title . "\" Due on " . $dueDate . " at " . $dueTime . ". Don't miss it! Have a nice day!";
